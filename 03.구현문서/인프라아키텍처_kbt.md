...
**버전**: v1.4-kbt
**근거 문서**: `02.기획문서/화면설계서_kbt.md`(MTRM-MAIN 포함 15개 화면, kbt 7차부터 FLOW-MAP·DASH-AUTO·DASH-MTRM 제외), `02.기획문서/API스펙_kbt.md`(I/F 설계서, IF-001~IF-009), `03.구현문서/시스템정의서_kbt.md`(Java Spring/Oracle 19.0 시스템 구성), `PRD.md`(부록 J 비기능요구사항)
...
>
> **kbt 검토 결과(2026-09-02, 3차 — 변경 없음 확인)**: SCR-004 화면 변경(`화면설계서_kbt.md` 6차, "사용현황(연동모듈수)"→"등록자" 전환, 수정 버튼 신규)은 프론트엔드 화면/데이터 표현 범위의 변경으로, 인프라 구성·용량·배포 전략에 영향이 없습니다. 본 절 이하 내용은 2차본과 동일하게 유지합니다.
>
> **kbt 검토 결과(2026-09-02, 4차 — 변경 없음 확인)**: FLOW-MAP·DASH-AUTO·DASH-MTRM 3개 화면이 프론트 코드에서 삭제되었습니다(`화면설계서_kbt.md` 7차 참고). 1차에서 서술한 대로 이 화면들은 서버 증설·신규 I/F·신규 DB 인스턴스 없는 순수 조회 화면이었으므로, 삭제 역시 인프라 구성·용량·배포 전략에 영향이 없습니다. 본 절 이하 내용은 3차본과 동일하게 유지합니다.
