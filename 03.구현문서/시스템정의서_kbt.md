...
>
> **kbt 변경 이력(2026-09-02, 3차 — 변경 없음 확인)**: SCR-004 화면 변경(`화면설계서_kbt.md` 6차, F-007 처리범위 "등록/삭제"→"등록/수정/삭제" 확장)은 표준작업명 모듈(3절, F-007/F-008)의 기존 Controller/Service/DAO 계층 내에서 처리 가능한 범위이며, 신규 모듈·신규 API·신규 I/F가 없습니다. 본 문서 내용은 2차본과 동일하게 유지합니다.
>
> **kbt 변경 이력(2026-09-02, 4차 — FLOW-MAP/DASH-AUTO/DASH-MTRM 3개 화면 삭제 반영)**: 프론트 코드에서 3개 화면이 삭제됨에 따라(`화면설계서_kbt.md` 7차 참고), 1차에서 추가했던 **대시보드 모듈**의 역할 범위를 MTRM-MAIN 전용으로 축소했습니다(3절, 4절 참고). 대시보드 모듈 자체는 삭제하지 않습니다 — MTRM-MAIN이 이 모듈(로드맵/Tech PR/기술동향 요약 조합)을 계속 사용 중입니다.
...
| **대시보드 모듈 (kbt 신규, 7차 축소)** | MTRM-MAIN(mTRM 요약) 조회 전용 조립 계층 — 신규 DB 테이블/신규 업무 로직 없이 하위 모듈 Service 결과만 조합 | F-010, F-011, F-013, F-019(전체 조회 재사용) | mTRM-로드맵 모듈, mTRM-TechPR 모듈, mTRM-기술동향 모듈 |

> 자동화현황(F-001~F-009, 1차 오픈 2026.09월말)과 mTRM(F-010~F-019, 2차 오픈 2026.12월말)은 오픈 차수가 다르므로, 공통 모듈을 우선 개발하여 두 트랙에서 공유하는 것을 권장합니다.
>
> **kbt 7차**: FLOW-MAP·DASH-AUTO·DASH-MTRM 3개 화면이 프론트 코드에서 삭제됨에 따라(`화면설계서_kbt.md` 7차 참고), 대시보드 모듈이 조립하던 대상에서 이 3개 화면 몫(F-001/F-005/F-009/F-016 재사용 조합, 자동화현황-부품공정/모듈공정 모듈·I/F연동 모듈·mTRM-협의체 모듈 의존)을 제거했습니다. 대시보드 모듈은 MTRM-MAIN 전용으로 축소되어 남습니다(신규 모듈 삭제는 아님 — MTRM-MAIN이 계속 사용 중).
...
│       ├── dao
│       └── vo
├── dashboard                        # 대시보드 모듈 (kbt 신규, 7차 축소 — MTRM-MAIN 전용, FLOW-MAP/DASH-AUTO/DASH-MTRM는 화면 삭제로 제외)
...
**작성 완료 여부**: [x] 시스템 정의서 작성 완료(v1.0 공식) / [x] kbt 1차·2차 반영 완료(대시보드 모듈 신규 추가) / [x] kbt 3차 반영 완료(SCR-004 변경 영향 없음 확인) / [x] kbt 4차 반영 완료(대시보드 모듈 MTRM-MAIN 전용으로 축소)
...
- `02.기획문서/화면설계서_kbt.md` (MTRM-MAIN 포함 총 15개 화면, kbt 7차부터 FLOW-MAP·DASH-AUTO·DASH-MTRM 제외)
