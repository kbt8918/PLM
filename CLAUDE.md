...
- **특정 화면군(SCR-XXX~YYY)을 SCR-001 방식으로 확장하는 작업을 마친 뒤에는, 요청받은 범위 밖의 다른 화면(특히 아직 손대지 않은 SCR)에도 "원본만 공통레이아웃 배열에 있고 그 화면의 팝업/파생 페이지 id는 빠져 있는" 동일 패턴이 있는지 `grep -oP "'scr-0\d\d[^']*'" | sort -u`로 전체 목록을 뽑아 `SBPPTX_COMMON_LAYOUT_SCREEN_IDS` 배열과 대조하는 전수 점검을 한 번 수행할 것**(2026-08-27 SCR-014 4개 파생 페이지 — selectmode/popup-trendedit/detail/popup-register — 가 원본 확장 시점부터 계속 빠져 있다가 사용자가 실제로 그 화면을 열어보고서야 발견됨, "SCR-001~008 회귀 없음"만 확인하고 범위 밖 화면의 기존 결함은 점검하지 않아 놓침). 이 점검은 새 화면군을 확장할 때마다가 아니라, **한 화면군 작업이 끝나고 다음 작업으로 넘어가기 전에 한 번**, 또는 사용자가 "이전 작업 영향 없는지" 물었을 때 요청 범위(SCR-001~008 등)만이 아니라 **전체 화면 id 목록**을 훑는 방식으로 수행한다.
